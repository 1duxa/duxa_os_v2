...
pub unsafe fn walky_talky(
    ...
    let p3 = phys_to_virt(PhysAddr::new(entry_addr(p4e)), phys_map).raw() as *mut PageTable;
    ...
    let p2 = phys_to_virt(PhysAddr::new(entry_addr(p3e)), phys_map).raw() as *mut PageTable;
    ...
    let p1 = phys_to_virt(PhysAddr::new(entry_addr(p2e)), phys_map).raw() as *mut PageTable;
...
pub fn phys_to_virt(phys: PhysAddr, phys_map: PhysAddr) -> VirtAddr {
    VirtAddr::new((phys + phys_map).raw())
}

pub fn virt_to_phys(virt: VirtAddr, phys_map: PhysAddr) -> PhysAddr {
    PhysAddr::new((virt - phys_map.raw()).raw())
}
